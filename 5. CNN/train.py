...
def get_model(config): #get model함수를 만듦.
    if config.model == 'fc': #모델이 만약 fc이면
        model = FullyConnectedClassifier(28**2, 10) #기존의 fullconnectedclassifier해주고 
    elif config.model == 'cnn': #모델이 cnn이면
        model = ConvolutionalClassifier(10) #convolutionalclassifer해줘라 라는 의미
    else:
...
def main(config):
    ...
    model = get_model(config).to(device) #그리고 위에서 선언한 모델을 받아온다.
